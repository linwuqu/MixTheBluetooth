...
public class DeviceListAdapter extends RecyclerCommonAdapter<DeviceModule> {

    public DeviceListAdapter(Context context, List<DeviceModule> strings, int layoutId){
    ...
    protected void convert(ViewHolder holder, DeviceModule item, int position, ItemClickListener itemClickListener) {
        ...
        if (item.getIBeacon() != null){
            holder.setText(R.id.item_main_beacon,"(iBeacon)")
                    .setText(R.id.item_main_distance,"距离:"+
                            new DecimalFormat("######0.00").format(item.getIBeacon().distance)+"米");
        }else {
            holder.setText(R.id.item_main_beacon,"")
                    .setText(R.id.item_main_distance,"");
        }
    }
}
