...
public abstract class BaseActivity<T extends ViewBinding> extends AppCompatActivity implements View.OnClickListener {
    ...
    protected final void onCreate(Bundle savedInstanceState) {
        ...
        /*
         * Thread类是JVM中用来管理线程的一个类,currentTread()返回现在正在执行的线程信息，setUncaughtExceptionHandler捕获线程异常
         * */
        Thread.currentThread().setUncaughtExceptionHandler(new MyUncaughtExceptionHandler());
        /*
        viewBinding来自getViewBinding(),而这个方法来着类T，类T继承ViewBinding，这会有个View类，
        view类是Android的一个超类，这个类几乎包含了所有的屏幕类型，每一个view都有一个用于绘画 的画布，这个画布可以进行任意的扩展，一个view占用一个矩形框。
        * */
        viewBinding = getViewBinding();
        setContentView(viewBinding.getRoot());
        /*
         * 定义一个抽象方法，执行功能从这里开始,就是这个initAll()方法
         * */
        initAll();
    }
    ...
    /**
     * @param key  由key值识别保存 将数据传递到下一个Activity中的数据
    ...
    protected <E> E getData(@NonNull String key, Class<E> type, Object... defaultValue) {
        ...
        if (String.class.equals(type)) data = getIntent().getStringExtra(key);
            ...
            data = getIntent().getIntExtra(key, defaultValue.length > 0 ? (int) defaultValue[0] : 0);
        else if (Float.class.equals(type))
            data = getIntent().getFloatExtra(key, defaultValue.length > 0 ? (float) defaultValue[0] : 0);
        else if (Character.class.equals(type))
            data = getIntent().getCharExtra(key, defaultValue.length > 0 ? (char) defaultValue[0] : '0');
        else if (Boolean.class.equals(type))
            data = getIntent().getBooleanExtra(key, defaultValue.length > 0 && (boolean) defaultValue[0]);
        else if (Double.class.equals(type))
            data = getIntent().getDoubleExtra(key, defaultValue.length > 0 ? (double) defaultValue[0] : 0);
        else if (Byte.class.equals(type))
            data = getIntent().getByteExtra(key, defaultValue.length > 0 ? (byte) defaultValue[0] : (byte) 0);
        else if (Long.class.equals(type))
            data = getIntent().getLongExtra(key, defaultValue.length > 0 ? (long) defaultValue[0] : 0);
        else if (Short.class.equals(type))
            data = getIntent().getShortExtra(key, defaultValue.length > 0 ? (Short) defaultValue[0] : (short) 0);
            ...
            throw new RuntimeException("没有收录" + type.getSimpleName() + "类型，请使用getIntent().get" + type.getSimpleName() + "Extra() 方法获取数值");
        return (E) data;
    }
