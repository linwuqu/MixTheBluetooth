...
| 能力 | 端口 | 底层 | 现状 |
|---|---|---|---|
| 蓝牙扫描 | `BluetoothPort.scanDevices()` | Android 原生 `BluetoothLeScanner`（callbackFlow + 单活动扫描互斥） | 已落地（05） |
| 蓝牙连接 | `BluetoothPort.execute()` | HC 库 `HcBluetoothLibraryClient` | 已落地 |
| 本地 KV | `LocalPort`（entropy） | SharedPreferences + Tink AES-GCM | 已落地 |
| 本地数据库 | LocalDatabase | SQLDelight（deviceBinding 表） | 已落地 |
| 本地文件 | `LocalPort`（file） | Okio | 已落地 |
...
| 日期 | 变更 |
|---|---|
| 2026-08-06 | 建立本文件；词汇流按 07 修订（Command/Result → Effect/Event） |
