...
> 词汇说明:本文沿用起草时的 Command/Result 名词。开工时统一为 04 修订后的 Effect(下行)/ Event(上行)词汇,与 `BluetoothPort` TODO 中预留的 `SendData / ObserveReceivedData` 等形态一致。
...
- Port 边界与前三章一致:统一为 `execute(effect): Flow<event>` 形态(04 修订后词汇,文中 Command/Result 为起草期名词,开工时替换)。
