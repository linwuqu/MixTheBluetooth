...
├─ port/
│  ├─ PortContracts.kt                          # CommandPort<Command, Result>(施工时保留,07 修订后删除,见 04-词汇统一)
│  ├─ auth/
│  │  ├─ AuthPort.kt                            # 契约不变
...
```kotlin
// port/upload/UploadPort.kt —— 业务契约(参照 AuthPort 模式,07 修订后词汇统一为 effect/event)
...
> 下方代码为施工时形态(词汇 `ConnectionCommand/ConnectionResult`);07 修订后签名统一为 `execute(effect: ConnectionEffect): Flow<ConnectionEvent>`,蓝牙命令包在 `ConnectDevice/DisconnectDevice` 内部,结构不变(见 04-词汇统一)。
